...
export default class Spreadsheet {
   ...
   */
	...
	await this._store.updateCell(this._name, cellId, formula);
      }
      return updates;
    }
    catch (err) {
  ...
  async clear() {
    if (this._store) await this._store.clear(this._name);
    this._undos = {};
    this._cells = {};
  }
  ...
  async delete(cellId) {
    ...
    if (this._store) await this._store.delete(this._name, cellId);
    return results;
  }
  ...
  async copy(destCellId, srcCellId) {
    ...
    if (!srcCell?.formula) {
      ...
      const destFormula = srcCell.ast.toString(destCellId);
